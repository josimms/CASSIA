...
carbo_tracker storage_carbohydrate_2(carbo_tracker critical_W, CASSIA_parameters parameters) {
  carbo_tracker out;
...
double storage_update_2(double alfa, double sugar, double starch, double Wala, bool tree_alive) {
  double out;
...
double storage_update_organs_2(double storage_capacity, double sugar, double starch, bool tree_alive) {
  double out;
  // The checks here make sense, as the model should compensate for the sugar lost at the end of each iteration rather than the beginning
  // Therefore, there should always be a positive value of sugar at the beginning of an iteration (and hopefully generally)
  if (std::isnan(sugar)) {
...
double nitrogen_storage_2(double sugar_mycorrhiza) {
  double out;
  out = std::max(0.0, 1/(1+exp(-4*(sugar_mycorrhiza - 0.2))));
  return(out);
}

carbo_tracker As_initiliser_2(carbo_tracker Ad, carbo_tracker equilibrium_temperature, double Bd, double Bs)
{
  carbo_tracker As;
...
double emergancy_2(double sugar, double starch, double tau_emergancy, double lower_bound) {
  double out;
  if (starch > 0) {
...
double respiration_check_2(double storage_term, double lower_bound) {
...
carbo_balance sugar_model_2(int year,
                          int day,
                          double TAir,
                          double PF,
  ...
  storage_term.needles = storage_update_organs_2(0.11*needles_mass, sugar.needles, starch.needles, tree_alive);
  storage_term.phloem = storage_update_organs_2(0.11*phloem_mass, sugar.phloem, starch.phloem, tree_alive);
  storage_term.xylem_sh = storage_update_organs_2(0.002*xylem_sh_mass, sugar.xylem_sh, starch.xylem_sh, tree_alive);
  storage_term.xylem_st = storage_update_organs_2(0.05*xylem_st_mass, sugar.xylem_st, starch.xylem_st, tree_alive);
  storage_term.roots = storage_update_organs_2(0.15*root_mass, sugar.roots, starch.roots, tree_alive);
  ...
  if (sperling_sugar_model) {
    if (day == 0) {
      ...
      storage_term_resp.needles = respiration_check_2(storage_term.needles, parameters.lower_bound_needles);
      storage_term_resp.phloem = respiration_check_2(storage_term.phloem, parameters.lower_bound_phloem);
      storage_term_resp.xylem_sh = respiration_check_2(storage_term.xylem_sh, parameters.lower_bound_xylem_sh);
      storage_term_resp.xylem_st = respiration_check_2(storage_term.xylem_st, parameters.lower_bound_xylem_st);
      storage_term_resp.roots = respiration_check_2(storage_term.roots, parameters.lower_bound_roots);
      ...
      if (surplus_c) {
        ...
        // storage_term.xylem_sh + storage_term.xylem_st
        double normalised_nitrogen_target_increase = std::max((storage_term.needles + storage_term.phloem + storage_term.roots)/5 - nitrogen_capacity, 0.0);
        // Sugar needed to make this difference
        double nitrogen_target_increase_sugar_investment = (1/4) * std::log((1/nitrogen_capacity - 1)/(1/(nitrogen_capacity+normalised_nitrogen_target_increase)-1));
        ...
        double total_use = resp.RmR * storage_term_resp.roots + (1 + common.Rg_R) * pot_growth.roots * storage_term.roots +
          resp.RmN * storage_term_resp.needles + (1 + common.Rg_N) * storage_term.needles * (pot_growth.needles + pot_growth.bud) +
          phloem_respiration_share * resp.RmS * storage_term_resp.phloem + phloem_respiration_share * (1 + common.Rg_S) * storage_term.phloem * (pot_growth.wall + pot_growth.height);
          //xylem_sh_respiration_share * resp.RmS * storage_term_resp.xylem_sh + xylem_sh_respiration_share * (1 + common.Rg_S) * storage_term.xylem_sh * (pot_growth.wall + pot_growth.height) +
          //xylem_st_respiration_share * resp.RmS * storage_term_resp.xylem_st + xylem_st_respiration_share * (1 + common.Rg_S) * storage_term.xylem_st * (pot_growth.wall + pot_growth.height);
      ...
      if (nitrogen_change) {
        nitrogen_capacity = nitrogen_storage_2(sugar.mycorrhiza);
      }
      ...
      sugar.roots = sugar.roots -
        ...
        (1 + common.Rg_R) * pot_growth.roots * std::min(storage_term.roots, nitrogen_capacity) -
      ...
      if (nitrogen_contrast) {
        if (sugar.roots < 0.0) {
          double sugar_demand_roots = 0 - sugar.roots;
          concentration_gradient.phloem_to_roots = std::min(sugar_demand_roots, sugar.phloem + starch.phloem);
        } // Else it would be the storage driven movement calculated earlier
      }

      sugar.roots = sugar.roots + concentration_gradient.phloem_to_roots;
      ...
      if (nitrogen_contrast) {
        if (sugar.phloem < 0.0) {
          double sugar_demand_phloem = 0 - sugar.phloem;
          concentration_gradient.needles_to_phloem = std::min(sugar_demand_phloem, sugar.needles + starch.needles);
        } // Else it would be the storage driven movement calculated earlier
      }

      sugar.phloem = sugar.phloem + concentration_gradient.needles_to_phloem;
      ...
      growth = std::min(storage_term.needles, nitrogen_capacity) * (pot_growth.needles + pot_growth.bud) +
        phloem_respiration_share * std::min(storage_term.phloem, nitrogen_capacity) * (pot_growth.wall + pot_growth.height) +
        xylem_st_respiration_share * std::min(storage_term.xylem_st, nitrogen_capacity) * (pot_growth.wall + pot_growth.height) +
        xylem_sh_respiration_share * std::min(storage_term.xylem_sh, nitrogen_capacity) * (pot_growth.wall + pot_growth.height) +
        std::min(storage_term.roots, nitrogen_capacity) * pot_growth.roots;
      ...
      emergancy_transfer.needles =  emergancy(sugar.needles, starch.needles, parameters.tau_emergancy, parameters.lower_bound_needles);
      emergancy_transfer.phloem = emergancy(sugar.phloem, starch.phloem, parameters.tau_emergancy, parameters.lower_bound_phloem);
      emergancy_transfer.xylem_sh = emergancy(sugar.xylem_sh, starch.xylem_sh, parameters.tau_emergancy, parameters.lower_bound_xylem_sh);
      emergancy_transfer.xylem_st = emergancy(sugar.xylem_st, starch.xylem_st, parameters.tau_emergancy, parameters.lower_bound_xylem_st);
      emergancy_transfer.roots = emergancy(sugar.roots, starch.roots, parameters.tau_emergancy, parameters.lower_bound_roots);
      ...
      if (day == 0) {
        parameters_in.As = As_initiliser_2(parameters_in.Ad, temperature_equilibrium, parameters_in.Ad.B, parameters_in.As.B);
      }
