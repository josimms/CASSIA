...
ratios_p[1,] <- c(0.6, 0.6, 0.55) # form_factor
...
ratios_p[3,] <- c(0.8, 0.8, 0.8) # sapwood.share
ratios_p[4,] <- c(4.3, 1, 2.6) # height_growth_coefficient repeated value, Variö: leader shoot length / average measurement shoot length (average over years)
ratios_p[5,] <- c(1.6, 1, 0.8) # diameter_growth_coefficient repeated value
ratios_p[6,] <- c(5.5, 1.28, NA) # height_growth_coefficient repeated value max if growth decreases
ratios_p[7,] <- c(3.8, 0.88, NA) # height_growth_coefficient repeated value min if growth decreases
ratios_p[8,] <- c(1.9, 1.19, NA) # diameter_growth_coefficient repeated value max if growth decreases
ratios_p[9,] <- c(1.5, 0.94, NA) # diameter_growth_coefficient repeated value min if growth decreases
...
## parameters_p
######
parameters_p <- data.frame(matrix(nrow = 56, ncol = 3))
names(parameters_p) <- c("Hyde", "Lettosuo", "Väriö")
#### Respiration
# Needles Q10
parameters_p[1,] <- c(1.898,1.898,1.898)  # Q10.N
# Needles R0
parameters_p[2,] <- c(0.00267, 0.0020, 0.004005) # Rm.N
# Wood (stem + brances + transport roots) Q10
parameters_p[3,] <- c(1.74788, 1.74788, 1.74788) # Q10.S
# Wood (stem + brances + transport roots) R0
parameters_p[4,] <- c(5.5576 * 10^-5, 5.5576 * 10^-5, 0.000083364) # Rm.S
# Fine roots Q10
parameters_p[5,] <- c(2.5575, 2.9662, 2.0244) # Q10.R
# Fine roots R0
parameters_p[6,] <- c(0.00958, 0.0059, 0.00945) # Rm.R

#### Gorwth
# Lettosuo: (as for pine, needs to be changed!)
# root growth beginning
parameters_p[7,] <- c(-2.5, -2.5, -3) # sR0
...
parameters_p[11,] <- c(10, 10, 10) # HH0
parameters_p[12,] <- c(-1.359200388, -4.12008, -1.44) # sH0
parameters_p[13,] <- parameters_p[14,] <- c(8.226401284, 8.226401284, 9.0) # LH, LH0
parameters_p[15,] <- c(14.59636279, 12.8328, 10.0) # sHc ## TODO: This value will change if the divers are set to specific values

#### Needles
# Lettosuo: parameters_p from 2015 and 2016
# Hyde: parameters_p from 2008
parameters_p[16,] <- c(-8.37584, -3.56589, -5.6) # sN0
parameters_p[17,] <- parameters_p[18,] <- c(1.849493, 1.849493, 3.5) # LN, LN0 n_lenght/(0.5*sNc)
parameters_p[19,] <- c(5.263883, 7.60671, 4.327) # sNc ## TODO: This value will change if the divers are set to specific values
parameters_p[20,] <- c(1, 1, 1) # HN0, initial needle lenght mm

#### Diameter
# Lettosuo: parameters_p from 2007, 2008 and 2009, L not correct!
parameters_p[21,] <- c(-3.724083738, -3.5, -2.434161213) # sD0.Trad
parameters_p[22,] <- parameters_p[23,] <- c(1.293443902, 1.9, 2.9) # LD, LD0
parameters_p[24,] <- c(5.077004992, 5.2, 4.093829285) # sDc ## TODO: This value will change if the divers are set to specific values
parameters_p[25,] <- c(NA, 8.8, NA) # sDc.T.count
...
parameters_p[26,] <- c(10.68685877, 5.5, 10.25174759) # tau.Ee
# duration of late wood cell enlargement
parameters_p[27,] <- c(8.789131263, 4.8, 4.510400352) # tau.El
# duration of early wood cell wall formation
parameters_p[28,] <- c(25.29448857, 17.8, 51.60724145) # tau.We
# duration of late wood cell wall formation
parameters_p[29,] <- c(35.12148687, 19.2, 17.76015932) # tau.Wl

# GPP effect on the daily value of parameter LD
parameters_p[30,] <- c(5, 5, 5) # tau.GPP
...
parameters_p[31,] <- c(1.95, 1.8, 1.95) # Uggla

#### Buds
# Bud growth begins at 20.6.
parameters_p[32,] <- c(171, 171, 181) # sB0
parameters_p[33,] <- c(85, 85, 60) # sBc
parameters_p[34,] <- c(0.005, 0.005, 0.003) # LB
...
parameters_p[35,]<-c(35.7*10^-6, 32.1*10^-6, 30*10^-6) # cell.d.ew
# late wood cell diameter Havimo, M. et al. 2008, Silva Fennica
parameters_p[36,]<-c(24.2*10^-6, 27.5*10^-6, 20*10^-6) # cell.d.lw
...
parameters_p[39,]<- c(114/2, 1140/2, 1114/2) # cell.wall.density.ew
# kg C m-3 , 1500/1114kg m-3, Relationships between the intra-ring wood density assessed by X-ray densitometry and optical anatomical measurements in conifers. Consequences for the cell wall apparent density determination
parameters_p[40,]<-c(114/2, 1360/2, 1114/2) 	# cell.wall.density.lw
...
parameters_p[41,]<-c(2.61*10^-6,3.1*10^-6,2.61*10^-6) # wall.thickness.ew
# Havimo, M. et al. 2008, Silva Fennica
parameters_p[42,]<-c(5.23*10^-6,3.88*10^-6,5.23*10^-6) # wall.thickness.lw

# Comes from cell volume / tauE
# this is part of the equations for pine, should check this for spruce as well
parameters_p[43,] <- c(NA, 5.49*10^-13, NA) # cell.volume.growth.per.day.ew
# Comes from cell volume / tauE
parameters_p[44,] <- c(NA,4.62*10^-13, NA)  # cell.volume.growth.per.day.lw

parameters_p[45,] <- c(400, 400, 400)	# kg m-3, Scots pine, density_tree # TODO: this should be in common!
parameters_p[46,] <- c(0.5, 0.5, 0.5)	# kg kg-1, carbon_share # TODO: this should be in common!

parameters_p[47,] <- c(0.175, 0.175, 0.154) # D0, diameter in the beginning (m)	(Hyy and Lsuo 2015, Väriö 2001)
parameters_p[48,] <- c(17.9, 17.9, 9.5) #h0, height in the beginning (m)
...
# ca 225 gC m-2 (roots < 2 mm) in control and 300-350 in fertilized (Leppälammi-Kujansuu et al. 2013, Plant Soil)
parameters_p[49,] <- c(3, 5, 5) # n_age, needles stay alive for X years
# average needle length between years, mm
parameters_p[50,] <- c(34.241, 13, 39.2) # n_lenght, needle length is on average
# average length growth (with trend removed)
parameters_p[51,] <- c(309.0938, 309.0938, 120.00) # h_increment, mean height increment (for Väriö 2010-2015)
# m2 kg-1 ICOS 2018, half of total needle area / needle mass
parameters_p[52,] <- c(13, 5.5, 13) # SLA
parameters_p[53,] <- c(0.07446064, NA, 0.02) # LR0

# Repola parameters
parameters_p[54,] <- c(-6.303, NA, NA) # b0_repo
parameters_p[55,] <- c(14.472, NA, NA) # b1_repo
parameters_p[56,] <- c(-3.976, NA, NA) # b2_repo

parameters_p[57,] <- c(0.02, NA, NA) # lower_bound_needles
parameters_p[58,] <- c(0.03, NA, NA) # lower_bound_phloem
parameters_p[59,] <- c(0.05, NA, NA) # lower_bound_roots
parameters_p[60,] <- c(0.03, NA, NA) # lower_bound_xylem_sh
parameters_p[61,] <- c(0.1, NA, NA) # lower_bound_xylem_sh
parameters_p[62,] <- c(3, NA, NA) # tau_emergancy_needles
parameters_p[63,] <- c(3, NA, NA) # tau_emergancy_phloem
parameters_p[64,] <- c(3, NA, NA) # tau_emergancy_roots
parameters_p[65,] <- c(3, NA, NA) # tau_emergancy_xylem_sh
parameters_p[66,] <- c(3, NA, NA) # tau_emergancy_xylem_st

parameters_p[67,] <- c(0.01, NA, NA) # lower_bound_W
parameters_p[68,] <- c(3, NA, NA) # tau_emergancy

parameters_p[69,] <- c(0.109, NA, 0.109) # uk_repo TODO
parameters_p[70,] <- c(0.118, NA, 0.118) # eki_repo TODO
...
sperling_p[4,] <- c(0.037, NA, NA) # starch.phloem0
sperling_p[5,] <- c(0.034, NA, NA) # starch.xylem.sh0
sperling_p[6,] <- c(0.166, NA, NA) # starch.xylem.st0
sperling_p[7,] <- c(0.057, NA, NA) # starch.roots0
sperling_p[8,] <- c(0.087, NA, NA) # sugar.needles0
sperling_p[9,] <- c(0.27, NA, NA) # sugar.phloem0
sperling_p[10,] <- c(0.014, NA, NA) # sugar.roots0
sperling_p[11,] <- c(0.0249, NA, NA) # sugar.xylem.sh0
sperling_p[12,] <- c(0.021, NA, NA) # sugar.xylem.st0
sperling_p[13,] <- sperling_p[14,] <- sperling_p[15,] <- sperling_p[16,] <- sperling_p[17,] <- sperling_p[29,] <- c(0.0, 0.0, 0.0) # Wala
sperling_p[18,] <- c(0.4211, 0.4211, NA) # carbon.sugar
sperling_p[19,] <- c(0.4444, 0.4444, NA) # carbon.starch
sperling_p[20,] <- sperling_p[21,] <- sperling_p[22,] <- sperling_p[23,] <- sperling_p[24,] <- sperling_p[30,] <- c(3, 3, 3) # alfa
sperling_p[25,] <- c(2, 2, 2) # tau.s
sperling_p[26,] <- c(2, 2, 2) # tau.t
sperling_p[27,] <- c(0.3246781, 0.40, 0.40) # starch00
sperling_p[28,] <- c(0.4184208, 0.35, 0.35) # sugar00
sperling_p[31,] <- c(3, NA, NA)   # Q10s As taken from a crop study for the tree study this model is based on I am assuming that these numbers can be generalised here as well
sperling_p[32,] <- c(1.8, NA, NA)  # Q10d As taken from a crop study for the tree study this model is based on I am assuming that these numbers can be generalised here as well
sperling_p[33,] <- c(0.23, NA, NA) # SCb kg C, just above lowest value in starch and sugar 15_16 -- JS notes
sperling_p[34,] <- c(0.41, NA, 0.35) # sugar.level, 23/9/2015, Sugar, starch and sugar 15_16 -- JS notes kg C, "assumed to equal SC at senescence"
sperling_p[35,] <- c(0.017, NA, NA) # Ad0.needles
sperling_p[36,] <- c(0.008, NA, NA) # Ad0.phloem
sperling_p[37,] <- c(2e-04, NA, NA) # Ad0.roots
sperling_p[38,] <- c(2e-04, NA, NA) # Ad0.xylem.sh
sperling_p[39,] <- c(0.047, NA, NA) # Ad0.xylem.st
sperling_p[40,] <- c(0.197, NA, NA) # lamda.needles
sperling_p[41,] <- c(0.05301, NA, NA) # lamda.phloem
sperling_p[42,] <- c(0.211, NA, NA) # lamda.roots
sperling_p[43,] <- c(0.00401, NA, NA) # lamda.xylem.sh
sperling_p[44,] <- c(0.00401, NA, NA) # lamda.xylem.st
sperling_p[45,] <- c(0.729, NA, NA) # delta.needles
sperling_p[46,] <- c(0.832, NA, NA) # delta.phloem
