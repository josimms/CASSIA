...
The other settings of the model can be accessed by the function argument switches. These settings can be found in the help function for the CASSIA model.
...
out3 = CASSIA_cpp(processed_data$weather_original, site = "Hyde", parameters = parameters_updated)
```
...
Due to the current definition of the model the weather data should be in the same format as the default Hyytiälä data even if the PRELES model is not used to calculate the photosynthesis input. However the values in these columns can just be NA values.
