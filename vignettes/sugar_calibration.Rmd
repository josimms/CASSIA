...
objective_function <- function(par) {
  ...
  tryCatch({
    ...
    if (any(is.na(model_values) | is.infinite(model_values))) {
      print(":)")
...
legend("bottomright", c("Original Ouput", "Surplus C"), col = c("green", "red"), , cex = 0.5, bty = "n", lty = 1)
points(as.Date(paste0(height_df$date, "-12-31")), height_df$amount, col = "green", pch = as.numeric(as.factor(height_df$variable))+16)
points(as.Date(paste0(height_cl_df$date, "-12-31")), height_cl_df$amount, col = "gold", pch = as.numeric(as.factor(height_cl_df$variable))+16)
...
ylim = range(c(0, max(cumsum(CASSIA_preles_1998_2022_roots_surplus_c$Sugar$sugar_mycorrhiza)), max(cumsum(CASSIA_preles_1998_2022$Sugar$sugar_mycorrhiza))), na.rm = TRUE)
# Mycorrhizal allocation
...
ylim = range(0, max(cumsum(CASSIA_preles_1998_2022$Sugar$sugar_mycorrhiza)))
...
# TODO: This allocation is way too high...
...
plot(dates, 100*CASSIA_preles_1998_2022$Sugar$sugar_mycorrhiza/CASSIA_preles_1998_2022$Sugar$sugar, col = "green", type = "l", main = "Sugar % to mycorrhiza", ylab = "Sugar % to Mycorrhiza", xlab = "Years", ylim = c(0, 12))
